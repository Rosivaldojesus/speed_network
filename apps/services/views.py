...
def ServicosAgendados(request):
    ...
    if queryset:
        ...
        quant_agendados = Servico.objects.filter( Q(contato_servico__icontains=queryset)).filter(status_concluido='False').count()

    if startdate:
        agendados = Servico.objects.filter(Q(data_agendada__exact=startdate)).filter(status_concluido='False')
        quant_agendados = Servico.objects.filter(Q(data_agendada__icontains=startdate)).filter(status_concluido='False').count()
