...
def Index(request):
    ...
    mes =  Pagamento.objects.filter(data_pagamento=data_atual).annotate(month=TruncMonth('data_pagamento')).filter().values('month').annotate(c=Sum('valor_pagamento')).values('month', 'c').order_by('month')
